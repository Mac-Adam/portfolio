...
export const TweetHeader = ({ tweet }) => (
  <div className="flex flex-row justify-between tracking-tight">
    <div className="flex items-center space-x-2">
      ...
      <div>
        <a target="_blank" rel="noreferrer" className="flex items-center whitespace-nowrap font-semibold">
          {truncate(tweet.user.name, 30)}
          {<Verified className="ml-1 inline size-4 text-blue-500" />}
        </a>
        <div className="flex items-center space-x-1">
          <a target="_blank" rel="noreferrer" className="text-sm text-gray-500 transition-all duration-75">
            @{truncate(tweet.user.screen_name, 26)}
          </a>
        </div>
      </div>
    </div>
  ...
  <div className="break-words leading-normal tracking-tighter">
    {tweet.entities.map((entity, idx) => {
      switch (entity.type) {
        ...
        case "mention":
          return (
            <a key={idx} target="_blank" rel="noopener noreferrer" className="text-sm font-normal text-gray-500">
...
export const MagicTweet = ({ tweet, components, className, ...props }) => {
  return (
    <div
      className={cn(
        "z-10 bg-background shadow-xl relative flex max-w-xl w-full flex-col gap-2 overflow-hidden rounded-xl border p-4 backdrop-blur-md",
        className
      )}
      {...props}
    >
